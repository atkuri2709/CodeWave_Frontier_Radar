...
export default function DigestsPage() {
  ...
  const filteredDigests = useMemo(() => {
    ...
    return digests.filter(d =>
      ...
      (d.created_at && new Date(d.created_at).toLocaleDateString().includes(q))
    );
  }, [digests, searchQuery]);
          ...
          {filteredDigests.map(d => (
            <div key={d.id} className="glass-card flex flex-col p-5">
              <div className="flex items-start justify-between">
                ...
                <span className="text-[11px]" style={{ color: '#9ba2bc' }}>{d.created_at ? new Date(d.created_at).toLocaleString(undefined, { dateStyle: 'medium', timeStyle: 'short' }) : '—'}</span>
              </div>
